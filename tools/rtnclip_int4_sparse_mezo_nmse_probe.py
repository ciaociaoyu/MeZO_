#!/usr/bin/env python
"""Probe INT4 RTNClip true-nMSE under Sparse-MeZO-style masks.

This is a probe-only diagnostic. The current sparse default is the
task-gradient static sensitive-parameter mask; magnitude-ranked masks are kept
only as legacy ablations.
"""
...
def build_task_grad_static_masks(
    master: Dict[str, torch.Tensor],
    ...
    *,
    ...
    if ratio <= 0.0 or ratio > 1.0 or not math.isfinite(ratio):
        raise ValueError(f"sparse_ratio must be in (0,1], got {sparse_ratio}")
    quantized_set = set(quantized_names)
    eligible = [name for name in master if name in quantized_set]
    if not eligible:
        raise RuntimeError("task_grad_static mask has no quantized Linear.weight parameters")
    ...
    for name, tensor in master.items():
        ...
        if name in quantized_set:
            total_quant += int(tensor.numel())
        if name in eligible:
            grad = params[name].grad
            if grad is None:
                score = torch.zeros(int(tensor.numel()), device=tensor.device, dtype=torch.float32)
            else:
                score = grad.detach().float().square().reshape(-1)
            chunks.append(score)
            names.append(name)
            numels.append(int(tensor.numel()))
    scores = torch.cat(chunks)
    total_eligible = int(scores.numel())
    k = min(max(int(math.floor(ratio * total_eligible)), 1), total_eligible)
    if k >= total_eligible:
        threshold = scores.min()
    else:
        threshold = torch.kthvalue(scores, total_eligible - k + 1).values
    ...
    for name, numel in zip(names, numels):
        score = scores[offset : offset + numel]
        offset += numel
    ...
    stats = {
        ...
        "total_params_quantized_linear": total_quant,
        "active_frac_quantized_linear": active_quant / max(total_quant, 1),
        "mask_source": "task_gradient_square_before_probe_batch",
        "sparse_mask_threshold": float(threshold.detach().cpu()),
        "eligible_params_for_mask": total_eligible,
    }
    return masks, stats
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mask_strategies", nargs="+", choices=["lowest_abs", "highest_abs", "task_grad_static"], default=["task_grad_static"])
...
def main() -> None:
    ...
    config = {
        ...
        "sparse_selection": "percentile_per_layer weight magnitude or global task-gradient square",
    ...
    for batch_id in range(int(args.probe_batches)):
        ...
        for strategy in args.mask_strategies:
            for ratio in args.sparse_ratios:
                if strategy == "task_grad_static":
                    masks, mask_stats = build_task_grad_static_masks(
                        master,
                        params,
