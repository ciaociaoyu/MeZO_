#!/usr/bin/env python
"""Full-data INT4 RTNClip sparse probe.

This is probe-only. It reuses the shared-grid RTNClip oracle and computes
default true nMSE/corr for sparse unscaled task-gradient static masks.
"""
...
def default_true_derivative(params: Dict[str, torch.nn.Parameter], directions: Dict[str, torch.Tensor]) -> Optional[float]:
    ...
    for name, direction in directions.items():
        ...
        if param is None or param.grad is None:
            continue
        acc += (param.grad.detach().float() * direction.float()).double().sum()
        seen = True
...
def setting_label(mask_stats: Dict[str, object]) -> str:
    ratio = float(mask_stats.get("sparse_ratio", 0.0))
    return f"sparse_p{f'{ratio:g}'.replace('.', 'p')}_full"
...
def summarize_task(
    ...
    setting = setting_label(mask_stats)
    for h in h_grid:
        group = [r for r in records if abs(float(r["h"]) - float(h)) < 1e-18]
        h_summary = preflight.summarize_h(group)
        row = {
            "dataset": task,
            "setting": setting,
            "h": float(h),
            ...
            **h_summary,
            ...
            "default_fd_true_nmse": h_summary.get("fd_true_nmse"),
            "default_corr_fd_true": h_summary.get("corr"),
            "default_true_direction": "gTu",
            "d_true_effective_mean": mean_metric(group, "d_true_effective"),
            **mask_stats,
        }
    ...
    hstar = {
        ...
        "fd_true_nmse_metric": "default_dh_vs_gTu",
        "nMSE_default_metric": "default_dh_vs_gTu",
        "default_fd_true_nmse": best.get("fd_true_nmse") if best else None,
        "default_corr_fd_true": best.get("corr") if best else None,
        "default_true_direction": "gTu",
        **mask_stats,
    }
    if best:
...
def run_task(task_name: str, args: argparse.Namespace, output_dir: Path) -> Dict[str, object]:
    ...
    for h in h_grid:
        for k in range(int(args.directions)):
            ...
            d_true_effective = preflight.effective_true_derivative(params, master, states, directions, float(h))
            d_true = default_true_derivative(params, directions)
            ...
            record = {
                ...
                "d_half": d_half,
                "d_true": d_true,
                "d_true_default": d_true,
                "d_true_effective": d_true_effective,
                "fd_true_nmse_metric": "default_dh_vs_gTu",
                "nMSE_default_metric": "default_dh_vs_gTu",
                "default_true_direction": "gTu",
                **pert,
...
def write_report(output_dir: Path, task_summaries: List[Dict[str, object]], all_probe_rows: List[Dict[str, object]]) -> None:
    sparse_ratio = task_summaries[0].get("sparse_ratio", "unknown") if task_summaries else "unknown"
    lines = [
        f"# Full-Data INT4 Sparse p={sparse_ratio} Probe",
        "",
        f"Setting: INT4 G128 RTNClip shared-grid fake quantization, sparse highest-|w| mask, p={sparse_ratio}, unscaled directions.",
        "",
        "Default nMSE is `default_dh_vs_gTu`: finite-difference `d_h` compared with the original directional derivative `g^T u`.",
        "",
        "| dataset | window | empirical best h | best default nMSE | best default corr | default nMSE@1e-3 | default corr@1e-3 | membership@1e-3 |",
        "| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |",
    ]
    by_task_h = {}
    ...
    for summary in task_summaries:
        ...
        lines.append(
            ...
            f"{fmt(summary.get('h_empirical_min_nmse'))} | {fmt(summary.get('default_fd_true_nmse', summary.get('best_fd_true_nmse')))} | "
            f"{fmt(summary.get('default_corr_fd_true', summary.get('best_corr')))} | "
            f"{fmt(h1e3.get('default_fd_true_nmse', h1e3.get('fd_true_nmse')))} | "
            f"{fmt(h1e3.get('default_corr_fd_true', h1e3.get('corr')))} | "
    ...
    for task in [str(s["dataset"]) for s in task_summaries]:
        lines.extend([
            f"### {task}",
            "",
            "| h | default_fd_true_nmse | default_corr_fd_true | alignment | norm_ratio | active_frac |",
            "| ---: | ---: | ---: | ---: | ---: | ---: |",
        ])
        for row in [r for r in all_probe_rows if r["dataset"] == task]:
            lines.append(
                f"| {fmt(row.get('h'))} | {fmt(row.get('default_fd_true_nmse', row.get('fd_true_nmse')))} | "
                f"{fmt(row.get('default_corr_fd_true', row.get('corr')))} | "
