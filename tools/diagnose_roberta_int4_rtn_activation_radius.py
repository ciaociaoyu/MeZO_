#!/usr/bin/env python
"""Diagnose RoBERTa INT4 RTNClip finite-difference activation radius.

This mirrors ``diagnose_opt_int4_rtn_activation_radius.py`` but loads the
project RoBERTa prompt-finetuning path.  It compares:

- uniform-theory active rate;
- phase-theory active rate using real weight phase inside cached RTNClip grids;
- actual active rate using the project RTNClip quantizer.
"""
...
def scale_like_weight(state: smoke.RTNClipState, weight: torch.Tensor) -> torch.Tensor:
    groups, _, valid = smoke._group_view_2d(weight, state.group_size)
...
def load_checkpoint_if_present(path: str, master: Dict[str, torch.Tensor], device: torch.device) -> Optional[int]:
    if not path:
        return None
    state = torch.load(path, map_location="cpu")
    saved = state.get("master", state)
    for name, tensor in saved.items():
        if name in master:
            master[name] = tensor.to(device=device, dtype=master[name].dtype)
    return int(state.get("step", 0)) if isinstance(state, dict) else None
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint_master", default="", help="Optional checkpoint master.pt")
    ...
    parser.add_argument("--max_total_elements", type=int, default=5_000_000, help="0 means use all coordinates")
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for RoBERTa activation-radius diagnostic.")
    ...
    model, _train_loader, _dev_loader, data_args, _train_sampler = smoke.load_prompt_model_and_data(load_args, device)
    params = smoke.named_parameter_map(model)
    master = {name: p.detach().clone().to(dtype=torch.float16) for name, p in params.items() if p.detach().is_floating_point()}
    checkpoint_step = load_checkpoint_if_present(args.checkpoint_master, master, device)
    ...
    accum: Dict[float, Dict[str, float]] = {
        h: {"uniform_sum": 0.0, "phase_sum": 0.0, "actual_sum": 0.0, "chi_sum": 0.0, "n": 0.0, "chi_n": 0.0}
        for h in hs
    }
    ...
    with torch.no_grad():
        for module_idx, name in enumerate(q_names):
            ...
            state, stats = smoke.compute_rtnclip_state(name, weight, int(args.bitwidth), int(args.group_size))
            ...
            idx = sample_indices(int(weight.numel()), sample_counts[name], device, index_gen)
            ...
            for h in hs:
                ...
                if args.actual_mode == "full_q":
                    q_plus = smoke.quantize_with_state(weight.float().add(direction, alpha=float(h)), state)
                    q_minus = smoke.quantize_with_state(weight.float().add(direction, alpha=-float(h)), state)
                ...
                chi = (delta_s[chi_mask] / denom[chi_mask] - 1.0).square()
    ...
    summary = {
        ...
        "checkpoint_master": args.checkpoint_master,
