...
"""
扫描当前目录下的形如：
  SST-2-roberta-large-prompt-standard-k16-roberta-large-mezo-ftseed42-bs64-lr1e-6-eps1.78e-3-wd0-step50000-evalstep5000
...
def parse_eps_from_dirname(dirname: str) -> Optional[float]:
    m = re.match(SUBDIR_PATTERN, dirname)
    if not m:
        return None
    try:
...
def main():
    ...
    for dirname, eps_val in eps_dirs:
        ...
        print(f"[DEBUG] {dirname} | 找到 CSV 数量:", len(csv_files))
        if not csv_files:
        ...
        for csv_path in csv_files:
            try:
                print(f"[DEBUG] 正在读取 CSV: {csv_path}")
            ...
            # --- 核心修复：step 列强制转为数值，避免 matplotlib 误判为 category ---
            step_numeric = pd.to_numeric(df[step_col], errors="coerce")

            # 如果 step 列全是 NaN，说明确实不可用
            if step_numeric.isna().all():
                print(f"[跳过] {csv_path} 的 step 列无法转为数值：{step_col}")
                continue
            ...
            print(f"[DEBUG] {csv_path} | {LOSS_COL} dtype =", df[LOSS_COL].dtype)
            print(f"[DEBUG] {csv_path} | {LOSS_COL} head =", df[LOSS_COL].head(5).tolist())

            # --- 核心修复：NO -> NaN -> 用上一个有效值 ---
            loss_numeric = pd.to_numeric(df[LOSS_COL], errors="coerce")

            # forward fill：NO 用上一个 loss
            loss_filled = loss_numeric.ffill()

            # 如果一开始就是 NO（前面没值），用后面的第一个有效值兜底
            loss_filled = loss_filled.bfill()
            ...
            if win > 1:
                ...
                loss_smoothed = loss_filled.to_numpy()

            fig, ax = plt.subplots(figsize=(7, 4))
            ax.plot(step_numeric.values, loss_smoothed, linewidth=1.6)
            ...
            if EVAL_LOSS_COL in df.columns:
                try:
                    ...
                    print(f"[ERROR] 计算 best_eval_loss 失败: {csv_path} | 错误: {e}")
                    best_eval = None
            else:
                best_eval = None
            ...
            try:
                best_train = float(np.nanmin(loss_filled.values))
            except Exception as e:
                print(f"[ERROR] 计算 best_train_loss 失败: {csv_path} | 错误: {e}")
                best_train = None
