...
RESULT_ROOT="${RESULT_ROOT:-${ROOT_DIR}/runs/next_round_residual_local_$(timestamp)}"
...
SEED="${SEED:-0}"
RUN_OPTIONAL_REPEAT="${RUN_OPTIONAL_REPEAT:-0}"
...
write_manifest() {
  python - "${RESULT_ROOT}" "${SEED}" "${EVAL_EVERY}" "${RUN_OPTIONAL_REPEAT}" <<'PY'
import json
import sys
...
run_optional_repeat = str(sys.argv[4]).lower() in {"1", "true", "yes"}
...
specs = [
    ("residual_grid_round_step1_lr5e-5_clip5_step500", "5e-5", 5),
    ("residual_grid_round_step1_lr1e-4_clip2_step500", "1e-4", 2),
    ("residual_grid_round_step1_lr7e-5_clip3_step500", "7e-5", 3),
]
if run_optional_repeat:
    specs.append(("residual_grid_round_step1_lr1e-4_clip5_step500_repeat", "1e-4", 5))

cases = []
for run_name, lr, clip in specs:
    item = dict(base)
    item.update({
        "run_name": run_name,
        "lr": lr,
...
manifest = {
    ...
    "mode": "next_round_residual_local",
    "cases": cases,
}
...
PY
  ...
  local log_path="${RESULT_ROOT}/logs/${RUN_NAME}.log"
  {
    ...
    (
      ...
      EXTRA_TAG=next-round-residual-local \
  ...
  python scripts/summarize_next_round.py "${RESULT_ROOT}" || true
}

write_manifest
{
  echo "# Next-round residual local commands"
  echo "RESULT_ROOT='${RESULT_ROOT}' CUDA_VISIBLE_DEVICES='${CUDA_VISIBLE_DEVICES}' RUN_OPTIONAL_REPEAT='${RUN_OPTIONAL_REPEAT}' bash scripts/run_next_round_residual_local.sh"
...

{
  ...
  echo "run_optional_repeat=${RUN_OPTIONAL_REPEAT}"
...
echo "local_sequential_next_round cases=${num_cases}" | tee "${RESULT_ROOT}/job_ids.txt"
for ((i = 0; i < num_cases; i++)); do
  run_case "${i}"
done

python scripts/summarize_next_round.py "${RESULT_ROOT}"
...
echo "AUTO_PROMOTE=1 requested, but promotion is intentionally manual after summary inspection." | tee -a "${RESULT_ROOT}/job_ids.txt"
