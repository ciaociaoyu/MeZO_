...
EXPERIMENT_ROOT="${SCRATCH_ROOT}/experiments/main/_shared/h_sweep_14h"
...
cd "${SCRATCH_ROOT}/experiments/main"

echo "Submitting Sparse MeZO 16-bit MNLI 14-value search"
mnli_job_id="$(submit_job "mnli" "sparse_mezo/roberta-large/mnli/fp16/h_sweep_14h/jobs/roberta_mnli_sparse_mezo16_14h.sh" "${BASE_DEPENDENCY_JOB_ID}")"
echo "Submitting Sparse MeZO 16-bit SST-5 14-value search with dependency afterany:${mnli_job_id}"
sst5_job_id="$(submit_job "sst5" "sparse_mezo/roberta-large/sst5/fp16/h_sweep_14h/jobs/roberta_sst5_sparse_mezo16_14h.sh" "${mnli_job_id}")"
