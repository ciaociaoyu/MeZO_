...
def main():
    ...
    # 6.1) 打印最佳 eval_loss 及其对应的 h 值
    try:
        best_idx = agg["best_eval_loss"].idxmin()
        best_row = agg.loc[best_idx]
        best_h = float(best_row["eps"])
        best_eval = float(best_row["best_eval_loss"])
        print(f"[信息] 最佳 eval_loss = {best_eval:.6g} 对应 h = {best_h:.6g}")
    except Exception as e:
        print(f"[警告] 计算最佳 eval_loss 时出错：{e}")
