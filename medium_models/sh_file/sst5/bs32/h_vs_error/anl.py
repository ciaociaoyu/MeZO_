    ...
    with jsonl_path.open("r", encoding="utf-8") as f:
        for line_no, line in enumerate(f, 1):
            ...
            if dt.size > 0 and df.size == dt.size:
                ...
                # d_fd ≈ a * d_true (least-squares slope through origin)
                denom = float(np.sum(dt * dt))
                if denom > 0.0:
                    slope_a = float(np.sum(dt * df) / denom)

            h_val = float(r.get("h", float("nan")))
            h_eff_mean = float(r.get("h_eff_mean", float("nan")))
            h_eff_over_h = float("nan")
            if math.isfinite(h_val) and h_val > 0.0 and math.isfinite(h_eff_mean):
                h_eff_over_h = float(h_eff_mean / h_val)
            ...
            rows.append(
                {
                    ...
                    # quantization/noise diagnostics
                    "delta_zero_frac": float(r.get("delta_zero_frac", float("nan"))),
                    "param_changed_ratio_mean": float(r.get("param_changed_ratio_mean", float("nan"))),
                    "h_eff_mean": h_eff_mean,
                    "h_eff_over_h": h_eff_over_h,
                }
            )
...
def summarize_by_h(df: pd.DataFrame) -> pd.DataFrame:
    agg = (
        ...
        .agg(
            ...
            slope_a_mean=("slope_a", "mean"),
            slope_a_std=("slope_a", "std"),
            deltaL_mean=("deltaL", "mean"),
            deltaL_std=("deltaL", "std"),
            delta_pred_mean=("delta_pred", "mean"),
            delta_pred_std=("delta_pred", "std"),
            delta_zero_frac_mean=("delta_zero_frac", "mean"),
            delta_zero_frac_std=("delta_zero_frac", "std"),
            param_changed_ratio_mean=("param_changed_ratio_mean", "mean"),
            param_changed_ratio_std=("param_changed_ratio_mean", "std"),
            h_eff_mean=("h_eff_mean", "mean"),
            h_eff_std=("h_eff_mean", "std"),
            h_eff_over_h_mean=("h_eff_over_h", "mean"),
            h_eff_over_h_std=("h_eff_over_h", "std"),
        )
        .reset_index()
        .sort_values("h")
    )
...
def plot_delta_zero_frac_vs_h(agg: pd.DataFrame, outdir: Path) -> None:
    x = agg["h"].to_numpy()
    y = agg["delta_zero_frac_mean"].to_numpy()
    ...
    plt.ylim(-0.05, 1.05)
    plt.grid(True, which="both", linestyle="--", alpha=0.4)
    plt.title("delta_zero_frac vs h")
    plt.xlabel("h")
    plt.ylabel("Mean frac(Δ=0)")
    plt.tight_layout()
    plt.savefig(outdir / "delta_zero_frac_vs_h.png", dpi=160)
    plt.close()


def plot_param_changed_ratio_vs_h(agg: pd.DataFrame, outdir: Path) -> None:
    x = agg["h"].to_numpy()
    y = agg["param_changed_ratio_mean"].to_numpy()
    ...
    plt.ylim(-0.05, 1.05)
    plt.grid(True, which="both", linestyle="--", alpha=0.4)
    plt.title("param_changed_ratio_mean vs h")
    plt.xlabel("h")
    plt.ylabel("Mean changed-param ratio")
    plt.tight_layout()
    plt.savefig(outdir / "param_changed_ratio_vs_h.png", dpi=160)
    plt.close()


def plot_h_eff_over_h_vs_h(agg: pd.DataFrame, outdir: Path) -> None:
    x = agg["h"].to_numpy()
    y = agg["h_eff_over_h_mean"].to_numpy()

    plt.figure()
    plt.plot(x, y, marker="o")
    plt.xscale("log")
    plt.grid(True, which="both", linestyle="--", alpha=0.4)
    plt.title("h_eff / h vs h")
    plt.xlabel("h")
    plt.ylabel("Mean (h_eff / h)")
    plt.tight_layout()
    plt.savefig(outdir / "h_eff_over_h_vs_h.png", dpi=160)
    plt.close()


def plot_slope_a_vs_h(agg: pd.DataFrame, outdir: Path) -> None:
    x = agg["h"].to_numpy()
    y = agg["slope_a_mean"].to_numpy()

    plt.figure()
    plt.plot(x, y, marker="o")
    plt.xscale("log")
    plt.grid(True, which="both", linestyle="--", alpha=0.4)
    plt.title("d_fd ≈ a * d_true (slope a) vs h")
    plt.xlabel("h")
    plt.ylabel("Mean slope a")
    plt.tight_layout()
    plt.savefig(outdir / "slope_a_vs_h.png", dpi=160)
    plt.close()
...
def main():
    ...
    plot_delta_zero_frac_vs_h(agg, outdir)
    plot_param_changed_ratio_vs_h(agg, outdir)
    plot_h_eff_over_h_vs_h(agg, outdir)
    plot_slope_a_vs_h(agg, outdir)
