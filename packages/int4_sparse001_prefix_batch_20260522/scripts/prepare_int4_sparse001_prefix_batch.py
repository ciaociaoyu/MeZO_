    ...
    for task in TASKS:
        ...
        for policy, fixed_h, fixed_label in POLICIES:
            if policy == "hstar_ours":
                h = float(src["hstar_nearest_grid"])
                h_label = src["hstar_nearest_grid_label"]
            else:
                h = float(fixed_h)
                h_label = str(fixed_label)
            run_name = f"int4_sparsep0p01_highestabs_{task.replace('-', '')}_{policy}_h{h_label}_seed16_full_bs64_step20k"
            out.append({
                "run_name": run_name,
                "run_dir": str(output_root / "int4_hsearch" / "sparse_p0p01_highestabs" / run_name),
                "phase": "int4_hsearch",
                "task_name": task,
                "dataset": task,
                "dataset_mode": "full",
                ...
                "sparse_ratio": 0.01,
                "sparse_p": 0.01,
                "sparse_mask_strategy": "highest_abs",
                "mask_strategy": "highest_abs",
                "sparse_rescale": "none",
                ...
                "notes": "sparse p=0.01 highest_abs unscaled; default nMSE target is dh_vs_gTu",
            })
    return out
