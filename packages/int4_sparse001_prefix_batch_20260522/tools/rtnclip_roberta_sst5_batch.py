...
def inject_prefix_for_training(model, num_prefix: int = 5) -> Tuple[List[str], str]:
    smoke.add_medium_models_to_path(REPO_ROOT)
    from src.prefix import PrefixTuning  # noqa: E402

    # The real-activation initializer in medium_models uses hard-coded token
    # ids and can trigger CUDA index asserts with current tokenizer/model
    # versions. Random prefix init is the stable default path used here.
    PrefixTuning(model, num_prefix=int(num_prefix), reparam=False, float16=True, init_by_real_act=False)
    status = "prefix_random_init"
...
def sample_prefix_directions_for_step(
    ...
    for name, tensor in master.items():
        if name not in prefix_set:
            directions[name] = torch.zeros_like(tensor, dtype=torch.float16)
    return directions
...
def write_resume_command(run_dir: Path, config: Dict[str, object], manifest_path: Optional[str] = None) -> None:
    ...
    direction_args = (
        ...
        f" --sparse_mask_strategy {config.get('sparse_mask_strategy', 'highest_abs')}"
...
def make_train_config(args, run_dir: Path, run_name: str, bitwidth: int, h: float, h_label: str, steps: int, scale_refresh_k: int, phase: str) -> Dict[str, object]:
    ...
    return {
        ...
        "sparse_mask_strategy": str(getattr(args, "sparse_mask_strategy", "highest_abs")),
        "mask_strategy": str(getattr(args, "sparse_mask_strategy", "highest_abs")),
...
def train_one(
    ...
    config["sparse_mask_strategy"] = str(config.get("sparse_mask_strategy", config.get("mask_strategy", "highest_abs"))).strip().lower()
    ...
    if config["direction_mode"] == "sparse":
        ...
        config["quantized_forward_scope"] = "base_Linear.weight_only_prefix_fp16"
    ...
    if config["direction_mode"] == "prefix":
        prefix_names, prefix_status = inject_prefix_for_training(model, int(config.get("prefix_num", 5)))
    ...
    initial_master_for_masks = {name: p.detach().clone().to(device=device, dtype=torch.float16) for name, p in params.items() if p.detach().is_floating_point()}
    ...
    if direction_mode == "sparse":
        ...
        sparse_masks, sparse_stats = build_sparse_masks(
            initial_master_for_masks,
            sparse_ratio=float(config.get("sparse_ratio", 0.1)),
            quantized_names=q_names,
            mask_strategy=str(config.get("sparse_mask_strategy", "highest_abs")),
        )
        config.update(sparse_stats)
        config["mask_source"] = "initial_unperturbed_fp16_master_weight"
        smoke.write_json(run_dir / "run_config.json", config)
        smoke.write_json(run_dir / "run_manifest_row.json", config)
        log_to(
            run_dir,
        ...
        smoke.write_json(run_dir / "run_config.json", config)
        smoke.write_json(run_dir / "run_manifest_row.json", config)
        log_to(run_dir, f"prefix mode prefix_params={len(prefix_names)} active_params={config['active_params_all']}")
    ...
    with metrics_path.open("a", newline="", encoding="utf-8") as f:
        ...
        for step_idx in range(start_step, steps):
            ...
            if finite:
                finite_count += 1
                update_norm_last = smoke.update_master(master, directions, float(config.get("lr", args.lr)), d_h)
                smoke.restore_master(params, master)
